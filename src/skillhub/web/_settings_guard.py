"""Проверяет недоверенный JSON-запрос сохранения настроек."""
...
MAX_SETTINGS_BODY_BYTES = 1_048_576
...
class SettingsForbiddenError(SkillHubError):
    """Описывает отказ проверки источника или секретного токена настроек."""

    code = "settings_forbidden"
    status_code = 403
    public_message = "Изменение настроек запрещено."


class SettingsRequestError(SkillHubError):
    """Описывает некорректное JSON-тело запроса настроек."""
...
async def validate_settings_request(
    request: Request,
    expected_token: str,
) -> dict[str, object]:
    """Проверяет источник, размер JSON и секретный токен настроек.
    ...
    Returns:
        Полный набор значений из проверенного тела.

    Raises:
        SettingsForbiddenError: источник или токен не прошёл строгую проверку.
        SettingsRequestError: тело не является корректным JSON UTF-8.
    ...
    if type(payload.get("csrf_token")) is not str:
        raise SettingsForbiddenError
    return _require_values(payload)
...
def _reject_oversized_declared_body(request: Request) -> None:
    ...
    if declared_length < 0 or declared_length > MAX_SETTINGS_BODY_BYTES:
        raise RequestTooLargeError
...
def _parse_json_payload(body: bytes) -> dict[str, object]:
    try:
        ...
        raise SettingsRequestError from exc
...
def _require_values(payload: dict[str, object]) -> dict[str, object]:

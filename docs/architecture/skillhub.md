...
- Конфигурация не читает `.env`, допускает `SKILLHUB_ENVIRONMENT=development|test|production`, необязательные `SKILLHUB_USAGE_PATH` (по умолчанию `usage.sqlite3`) и `SKILLHUB_DAILY_BUDGET_NANOS` (дневной потолок в нано-USD; отсутствие переменной означает, что потолка нет), и отклоняет неизвестные `SKILLHUB_*` через `extra=forbid`. Цены моделей в окружение не выносятся. Ошибка настроек или сборки даёт фиксированное JSON-событие без исходного значения, трассировки и абсолютного пути, затем процесс завершается с кодом 2.

...
- As-built: `GET /protocol` показывает таблицу уточнений; `POST /protocol/clarifications`, `/answer` и `/finalize` собирают итоговый `Protocol` без повторной модели. `verify(protocol, material)` — чистая функция. На `/protocol` порт генерации в штатном `create_app` равен `None`.
